...
class DecisionTree(object):
    ...
    def get_training_data(self):
        ...
        for feature in self._feature_names:
            ...
            for value in values_for_feature:
                ...
                self._probability_cache[feature_and_value] = self.probability_for_feature_value(feature, value)
    ...
    def recursive_descent_for_classification(self, node, feature_and_values):
        ...
        for child in children:
            ...
            if last_feature_and_value_on_branch == feature_value_combo:
                answer = self.recursive_descent_for_classification(child, remaining_features_and_values)
                break
        return answer
    ...
    def recursive_descent(self, node):
        ...
        best_feature, best_feature_entropy =  self.best_feature_calculator(features_and_values_on_branch)
        ...
        if best_feature_entropy < node.get_entropy() - self._entropy_threshold:
            values_for_feature = self._features_and_values_dict[best_feature]
            feature_value_combos = map(lambda x: "".join([best_feature,"=>",x]), values_for_feature)
            ...
            for feature_and_value in feature_value_combos:
                ...
                if features_and_values_on_branch is None:
                    ...
                    extended_branch_features_and_values.append(feature_and_value)

                class_probabilities = list(map(lambda x: self.probability_for_a_class_given_sequence_of_features_and_values(x, extended_branch_features_and_values), self._class_names))

                child_node = Node(None, best_feature_entropy, class_probabilities, extended_branch_features_and_values)
    ...
    def probability_for_feature_value(self, feature, value):
        ...
        values_for_feature = self._features_and_values_dict[feature]
        values_for_feature = list(map(lambda x: feature + "=>" + x, values_for_feature))
    ...
    def probability_for_a_class_given_sequence_of_features_and_values(self, class_name, array_of_features_and_values):
        sequence = ":".join(array_of_features_and_values)
        ...
        for i in range(0, len(self._class_names)):
            prob = self.probability_for_sequence_of_features_and_values_given_class(array_of_features_and_values, self._class_names[i]) 
            if prob == 0:
...
class Node(object):
    ...
    def display_node(self):
        ...
        print("\n\nNODE " + str(serial_num) + ":\n   Branch features and values to this node: " + str(branch_features_and_values) + "\n   Class probabilities at current node: " + str(class_probabilities) + "\n   Entropy at current node: " + str(entropy_at_node) + "\n   Best feature test at current node: " + feature_at_node + "\n\n")
    ...
    def display_decision_tree(self, offset):
        ...
        if len(self.get_children()) > 0:
            ...
            entropy_at_node = self.get_entropy()
            class_probabilities = self.get_class_probabilities()
            print("NODE " + str(serial_num) + ":  " + offset +  "feature: " + feature_at_node + "   entropy: " + str(entropy_at_node) + "   class probs: " + str(class_probabilities) + "\n")
            offset += "   "
            ...
            entropy_at_node = self.get_entropy()
            class_probabilities = self.get_class_probabilities()
            print("NODE " + str(serial_num) + ": " + offset + "   entropy: " + str(entropy_at_node) + "    class probs: " + str(class_probabilities) + "\n")
